...
function rowResultKind(row: AttemptRow, now: Date): ResultKind {
  return resultKind(row.status, row.score, row.passingScore, row.startedAt, row.timeLimitMinutes, now);
}
          ...
          {sorted.map((row) => {
            ...
            return (
              <TableRow key={row.id}>
                ...
                <TableCell>
                  {row.score !== null && row.totalCorrect !== null && row.totalQuestions !== null
                    ? `${row.totalCorrect}/${row.totalQuestions} (${row.score}%)`
                    : "—"}
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <Link
                    href={`/dashboard/attempts/${row.id}`}
