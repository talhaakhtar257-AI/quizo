...
export function QuestionCard({
  ...
  async function handleSave() {
    ...
    try {
      await onSave({
        ...
        scenarioText: draft.scenarioText,
    ...
    return (
      <Card className="space-y-4 p-5">
        ...
        <Textarea
          ...
          onChange={(event) =>
            setDraft((current) => ({ ...current, scenarioText: event.target.value }))
          }
        />
        <Textarea
          label="Question"
  ...
  return (
    <Card className="space-y-3 p-5">
      ...
      <div className="rounded-md bg-info-bg p-3 text-sm text-fg">{question.scenario_text}</div>
      <p className="font-medium text-fg">{question.question_text}</p>
